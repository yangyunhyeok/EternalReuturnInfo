...
- FireBase(DB)
  ...
  - 앱 기능을 구현하기 위해 데이터 베이스가 필요했었고 ROOM, 서버 등 다양한 선택지가 있었으나 구현하려는 기능들이 다른 기기들과 실시간으로 데이터가 공유가 가능해야했기에 로컬에 저장하는 ROOM을 제외하였고 실시간 공유와 일정 사용량까지 무료로 제공해주는 점을 생각해 결론적으로 Firebase를 선택하게 되었습니다
